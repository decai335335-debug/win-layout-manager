...
| 步骤 | 匹配模板 | 置信度 | 缩放 | 状态 |
|---|---|---|---|---|
| 1. VS Code → left | `3column_layout.png` | 0.783 | 1.0x | ✅ |
| 2. Tabbit → center | `icon_tabbit.png` | 1.000 | 1.0x | ✅ |
| 3. Explorer → right | `icon_filemanager.png` | 1.000 | 1.0x | ✅ |
| 4. Kimi Code → top | `layout_2row.png` | 0.737 | 1.0x | ✅ |
| 5. QClaw → bottom | `icon_qclaw.png` | 0.543* | 1.0x | ✅ |

> *QClaw 修复前置信度 0.497（低于阈值失败），修复后 0.543 稳定通过

### 性能数据

| 指标 | v3.0 | v4.0 | 优化手段 |
|---|---|---|---|
| 全流程耗时 | 5~8 秒 | **3~4 秒** | 削减 sleep + 禁用调试截图 + fast-path |
| 单步截图 + 匹配耗时 | < 200ms | < 150ms | fast-path 1.0x 先尝试 |
| 模板匹配多尺度搜索 | 7 个尺度全遍历 | 1~2 个尺度 | 1.0x ≥0.70 直接 break |
| 虚拟桌面截图（mss） | < 100ms | < 100ms | 无变化 |
| 调试截图写磁盘 | 200~500ms/张 | **0ms**（已禁用） | `save_debug_screenshot` 改为 `pass` |
| 显式 sleep 总时长 | ~24 秒 | **~3.6 秒** | 逐个削减冗余等待 |

### v4.0 新增踩坑

| 问题现象 | 根因 | 解决方案 | 涉及版本 |
|---|---|---|---|
| QClaw 匹配失败（最佳=0.497）| Kimi Code snap 到 top 后，Win11 需要时间渲染 bottom 区域建议缩略图；0.2s 等待不够 | 等待时间 0.2→0.6s；恢复正确流程（直接点击缩略图，不额外 Win+Z） | v4.0 |
| VS Code 分屏没点出来 | Win+Z 弹出动画未完成就截图，匹配到错误位置（y=1415 屏幕底部） | Win+Z 后等待 0.1→0.4s，确保弹出框完全显示 | v4.0 |
| 全流程体感慢 | 大量冗余 sleep + 调试截图写磁盘是最大瓶颈 | 禁用调试截图；削减 click/ShowWindow/move_window sleep；删除 bring_to_front 后冗余等待 | v4.0 |
