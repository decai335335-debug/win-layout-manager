...
> 如果你换了一台新电脑、换了显示器、或者想尝试不同的窗口组合，以下功能会让这个项目越来越顺手。

### 近期 (v4.x)
- [x] **自动启动未打开的程序** —— `APP_CONFIGS` 配置字典，找不到窗口时自动启动对应程序
- [ ] **配置文件外部化** —— 将窗口标题、exe 路径、目标显示器、目标区域抽取到 `config.json` 或 `config.yaml`，不改代码就能切换整套布局
- [ ] **多布局预设切换** —— 内置多套预设（工作模式、写作模式、娱乐模式），运行时通过参数切换，例如 `python snap_all.py --preset writing`
- [ ] **更明确的错误提示** —— 区分"显示器未检测到"、"窗口未找到"、"模板匹配失败"，让用户一眼知道该修哪里

### 中期 (v5.x)
- [ ] **GUI 配置界面** —— 系统托盘右键菜单直接勾选当前打开的窗口，拖拽分配到左/中/右/上/下，无需手动编辑代码
- [ ] **一键保存当前布局** —— 把当前所有窗口的位置和大小保存为预设，下次一键还原（类似 Photoshop 工作区）
- [ ] **支持第三方窗口管理器** —— 除了 Win11 Snap Layouts，增加对 PowerToys FancyZones、DisplayFusion 的支持
- [ ] **全局热键触发** —— 托盘常驻后，按 `Ctrl + Alt + 1` 切工作模式、`Ctrl + Alt + 2` 切写作模式，无需打开终端
- [ ] **自动检测程序安装路径** —— 扫描开始菜单和注册表，自动填充 `APP_CONFIGS` 中的 exe 路径

### 远期 (v6.x)
- [ ] **智能布局推荐** —— 根据当前已打开的窗口列表，AI 推荐最合适的布局（比如检测到 Premiere + Audition 就推荐"剪辑模式"）
- [ ] **窗口分组与工作区** —— 类似 macOS Mission Control，把整个桌面（含窗口、标签页、甚至文件管理器路径）打包为一个"工作区"，一键切换
- [ ] **跨平台支持** —— 适配 Linux（KDE/GNOME 窗口管理）和 macOS（Rectangle / Magnet）
- [ ] **插件系统** —— 允许用户用 Python 编写自定义布局逻辑，例如"每天 9:00 自动切换到工作模式"
